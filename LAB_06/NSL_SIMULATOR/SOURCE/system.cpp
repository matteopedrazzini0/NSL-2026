...
void System :: move(int i){ // Propose a MC move for particle i
  ...
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double deltaE = _J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H;
    double prob = 1/(1+exp(-2*_beta*deltaE));
    //flippo con probabilità prob
...
double System :: Boltzmann(int i, bool xnew){
  ...
  for (int j=0; j<_npart; j++){
    if(j != i){
      ...
      dr = sqrt(dr);
      if(dr < _r_cut){
        energy_i += 1.0/pow(dr,16) - 1.0/pow(dr,6);
      }
    }
  }
  return 4.0 * energy_i;
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  couta << "#T:" << setw(16) << "ACCEPTANCE:" << endl;
  couta.close();
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutt << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_TE:" << setw(12) << "TE_AVE:" << setw(12) << "ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutte << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_T:" << setw(12) << "T_AVE:" << setw(12) << "ERROR:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        ...
        coutpr << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_P:" << setw(12) << "P_AVE:" << setw(12) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_M:" << setw(12) << "M_AVE:" << setw(12) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_CV:" << setw(12) << "CV_AVE:" << setw(12) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << "#T" << setw(12) << "BLOCK:" << setw(12) << "ACTUAL_X:" << setw(12) << "X_AVE:" << setw(12) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,16) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          if(_measure_pressure) virial       += 1.0/pow(dr,16) - 0.5/pow(dr,6); // PRESSURE
        }
      }
    }
  }
  ...
  if(_measure_pofv){
    for(int i{}; i<_npart; i++){
      double v_ppart = arma::norm(_particle(i).getvelocity()); //pofv_temp = 
      // find the corresponding bin
      int bin = int(v_ppart/_bin_size_v); //if the bin start from 0 to v_max=4T, the position of the in wich fall the velocity is given by the inversion relation, litteraly the velocity is given by the bin_size * bin = v
      if (bin<_n_bins_v && bin > 0) _measurement(_index_pofv+bin)++;
    }
    //normalization
  ...
  if(_measure_magnet){
    double mag_temp=0;
    for(int i=0;i<_npart;i++){
      mag_temp += _particle(i).getspin();
    }
    //salvo
    _measurement(_index_magnet)=mag_temp / double(_npart);
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    //qui calcolo solo H^2, perchè H lo calcola già nell'energia
      double tenergy2_temp = pow(_measurement(_index_tenergy), 2);
...
void System :: averages(int blk){
  ...
  if (_measure_pofv){
    ...
    for(int i{}; i<_n_bins_v; i++){ //simpy do the average process over forall bin, cause in each bin we have a measure, and its estimation is due with data blocking
      average = _average(_index_pofv+i);
      sum_average = _global_av(_index_pofv+i);
      sum_ave2 = _global_av2(_index_pofv+i);
      coutf << setw(16) << blk
            << setw(16) << (i+0.5) * _bin_size_v
            << setw(16) << _average(_index_pofv+i)
            << setw(16) << sum_average/double(blk)
            << setw(16) << this->error(sum_average, sum_ave2, blk)
            << endl;
    }
    coutf.close();
  }
  ...
  if(_measure_magnet){
    ...
    coutf << setw(16) << _temp
          << setw(16) << blk //blocco corrente
          << setw(16) <<average //M attuale
          << setw(16) << sum_average/(double)blk //media dei blocchi
          << setw(16) << this->error(sum_average, sum_ave2, blk) << endl; //errore
    coutf.close();
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    ...
    coutf << setw(16)<<_temp //temperatura
          << setw(16) << blk //blocco corrente
          << setw(16) << average //media di blocco
          << setw(16) << sum_average/double(blk)//media progressiva
          << setw(16) << this->error(sum_average, sum_ave2, blk) //errore
          << endl;
    coutf.close();
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  ...
  coutf << _temp << setw(16) << fraction << endl;
  coutf.close();
...
void System :: setTemp(double temp){
  _temp=temp;
  _beta=1.0/temp;
}

void System :: set_step(int step_number){
  _nsteps = step_number;
}

void System :: Reset_acceptance(){
  _nattempts=0;
  _naccepted=0;
}

void System :: reset_global(){
  _global_av.zeros();
  _global_av2.zeros();
  _average.zeros();
  _block_av.zeros();
