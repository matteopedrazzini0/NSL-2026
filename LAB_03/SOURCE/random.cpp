...
double Random :: retta(double m, double q, double xmin, double xmax){
   if(m==0) return Rannyu(xmin, xmax);
   double norm = 0.5 * m *(xmax*xmax - xmin*xmin) + q * (xmax-xmin);
   double y = Rannyu();
   double R = q*q + 2 * m * (y*norm + 0.5*m*xmin*xmin + q * xmin);
   return (-q + sqrt(R)) / m;
}
...
void Random :: Initialize_RNG(string file_Primes, string file_Seed){
   int seed[4];
   int p1, p2;
   ifstream Primes(file_Primes);
   if (Primes.is_open()){
      Primes >> p1 >> p2 ;
   } else cerr << "PROBLEM: Unable to open " << file_Primes << endl;
   Primes.close();

   ifstream input(file_Seed);
   string property;
   if (input.is_open()){
      while ( !input.eof() ){
         input >> property;
      ...
      input.close();
   } else cerr << "PROBLEM: Unable to open " << file_Seed << endl;
}
/****************************************************************
*****************************************************************
    _/    _/  _/_/_/  _/       Numerical Simulation Laboratory
