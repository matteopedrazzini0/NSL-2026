  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutgr << setw(14) << "#BLOCK" << setw(14) << "DISTANCE:" << setw(14) << "AVE_GOFR:" << setw(14) << "ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        coutpv << setw(14) << "#BLOCK" << setw(14) << "VELOCITY:" << setw(14) << "AVE_POFV:" << setw(14) << "ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
...
void System :: write_XYZ(int nconf){
  ...
  if(coutf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      coutf << "LJ" << "  " 
            << setw(14) << _particle(i).getposition(0,true)          // x
            << setw(14) << _particle(i).getposition(1,true)          // y
            << setw(14) << _particle(i).getposition(2,true) << endl; // z
    }
  } else cerr << "PROBLEM: Unable to open config.xyz" << endl;
  coutf.close();
  return;
...
void System :: averages(int blk){
  ...
  if(_measure_gofr){
        ...
        coutf << setw(14) << blk
              << setw(14) << (i+0.5) * _bin_size
              << setw(14) << average
              << setw(14) << sum_average / blk
  ...
  if (_measure_pofv){
    ...
    for(int i{}; i<_n_bins_v; i++){ //simpy do the average process over forall bin, cause in each bin we have a measure, and its estimation is due with data blocking
      ...
      coutf << setw(14) << blk
            << setw(14) << (i+0.5) * _bin_size_v
            << setw(14) << _average(_index_pofv+i)
  ...
  if(_measure_magnet){
    ...
    coutf << setw(14) << _temp
          << setw(14) << blk //blocco corrente
          << setw(14) <<average //M attuale
          << setw(14) << sum_average/(double)blk //media dei blocchi
          << setw(14) << this->error(sum_average, sum_ave2, blk) << endl; //errore
    coutf.close();
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  // TO BE FIXED IN EXERCISE 6
  if(_measure_cv){
    ...
    coutf << setw(14)<<_temp //temperatura
          << setw(14) << blk //blocco corrente
          << setw(14) << average //media di blocco
          << setw(14) << sum_average/double(blk)//media progressiva
          << setw(14) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
