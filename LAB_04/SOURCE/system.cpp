...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  while ( !input.eof() ){
    ...
    } else if(property=="DISTRIBUTION_TYPE"){
      input >> _distr_type;
      if(_distr_type==0) coutf << "INITIAL VELOCITY DISTRIBUTION = MAXWELL-BOLTZMANN" << endl;
      else if(_distr_type==1) coutf << "INITIAL VELOCITY DISTRIBUTION = DIRAC-DELTA" << endl;
      else{
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    if(_distr_type==0){ // M-B distribution
      vec sumv(_ndim);
      sumv.zeros();
      for (int i=0; i<_npart; i++){
      ...
      ofstream coutpvsi("../OUTPUT/initial_velocity_distr.dat");
      coutpvsi << "# V_BIN:" << setw(18) << "COUNTS:" << endl;
      double v = sqrt(3.0*_temp);
      for(int i=0; i<_npart; i++){
        int resto = i % 6 ;
        if(resto == 0){
          vx(i) = v, vy(i) = 0.0, vz(i) = 0.0;
        }
        else if(resto == 1){
          vx(i) = -v, vy(i) = 0.0, vz(i) = 0.0;
        }
        else if(resto == 2){
          vx(i) = 0.0, vy(i) = v, vz(i) = 0.0;
        }
        else if(resto == 3){
          vx(i) = 0.0, vy(i) = -v, vz(i) = 0.0;
        }
        else if(resto == 4){
          vx(i) = 0.0, vy(i) = 0.0, vz(i) = v;
        }
        else if(resto == 5){
          vx(i) = 0.0, vy(i) = 0.0, vz(i) = -v;
        }
      ...
      int num_bin = 20;
      double bin_siz_v = 4.0*_temp/(double)num_bin;
      vec v_in(num_bin);
      for(int i=0; i<_npart; i++){
        double v = arma::norm(_particle(i).getvelocity());
        int bin = int(v / bin_siz_v);
        if(bin<num_bin && bin>0) v_in(bin)++;
      }
      for(int j=0; j<num_bin; j++){
        coutpvsi << bin_siz_v * (j+0.5) << setw(18) << v_in(j) << endl;
      }
      coutpvsi.close();
    }
    for (int i=0; i<_npart; i++){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutp << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_PE:" << setw(16) << "PE_AVE:" << setw(16) << "ERROR:" << endl;
        coutp.close();
        _nprop++;
        ...
        coutk << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_KE:" << setw(16) << "KE_AVE:" << setw(16) << "ERROR:" << endl;
        coutk.close();
        _nprop++;
        ...
        coutt << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_TE:" << setw(16) << "TE_AVE:" << setw(16) << "ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutte << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_T:" << setw(16) << "T_AVE:" << setw(16) << "ERROR:" << endl;
        coutte.close();
        _nprop++;
        _measure_temp = true;
        ...
        coutpr << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_P:" << setw(16) << "P_AVE:" << setw(16) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutgr << "#" << setw(16) << "DISTANCE:" << setw(16) << "AVE_GOFR:" << setw(16) << "ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        coutpv << "#" << setw(16) << "BLOCK" << setw(16) << "VELOCITY:" << setw(16) << "AVE_POFV:" << setw(16) << "ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(3.0*_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4
        _measure_pofv = true;
        _index_pofv = index_property;
        index_property += _n_bins_v;
      } else if( property == "MAGNETIZATION" ){
        ofstream coutpr("../OUTPUT/magnetization.dat");
        coutpr << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_M:" << setw(16) << "M_AVE:" << setw(16) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        ...
        coutpr << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_CV:" << setw(16) << "CV_AVE:" << setw(16) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_cv = true;
        ...
        coutpr << "#" << setw(16) << "BLOCK:" << setw(16) << "ACTUAL_X:" << setw(16) << "X_AVE:" << setw(16) << "ERROR:" << endl;
        coutpr.close();
        _nprop++;
        _measure_chi = true;
